 ...
 *
 * <p>The button will ask the user if he wants to delete the received emails or not.<br>
...
public final class ClearAllButton extends Observable implements Observer {
   ...
   *
   * <p>The button will be disabled by default, since no email is received when the application
   * starts.<br>
   * The button will display a confirmation dialog to know if it needs to delete the received emails
   * or not.<br>
        ...
        e -> {
          int answer =
              JOptionPane.showConfirmDialog(
                  button.getParent(),
                  i18n.get("clearall.delete.email"),
                  String.format(
                      i18n.get("clearall.title"), Configuration.INSTANCE.get("application.name")),
          ...
          synchronized (SMTPServerHandler.INSTANCE.getMailSaver().getLock()) {
            // Note: Should delete emails before calling observers, since observers will clean the
            // model.
   ...
   *
   * <p>This method will be called by a {@link MailSaver} object when an email will be received.
   */
  @Override
