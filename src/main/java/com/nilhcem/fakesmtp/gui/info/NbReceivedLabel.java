...
public final class NbReceivedLabel implements Observer {
  ...
  public NbReceivedLabel() {
    Font boldFont =
        new Font(nbReceived.getFont().getName(), Font.BOLD, nbReceived.getFont().getSize());
   ...
   *
   * <ul>
   *   <li>If the observable element is a {@link MailSaver} object, the method will increment the
   *       number of received messages and update the {@link UIModel};
   *   <li>If the observable element is a {@link ClearAllButton}, the method will reinitialize the
   *       number of received messages and update the {@link UIModel};
  ...
  private void updateDockIconBadge(String badgeValue) {
    try {
      ...
      LOGGER.debug(
          "Error: {} - This is probably because we run on a non-Mac platform and these components"
              + " are not implemented",
          e.getMessage());
    } catch (Exception e) {
      LOGGER.error("", e);
