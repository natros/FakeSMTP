...
public final class MainFrame {
   ...
   *
   * <p>First, assigns the main panel to the default uncaught exception handler to display
   * exceptions in this panel.<br>
   * <br>
   * Before creating the main window, the application will have to set some elements, such as:
   *
   * <ul>
   *   <li>The minimum and default size;
   *   <li>The menu bar and the main panel;
   *   <li>An icon image;
   *   <li>A shutdown hook to stop the server, once the main window is closed.
   * </ul>
   *
   * <br>
   *
   * <p>The icon of the application is a modified version from the one provided in "{@code
   * WebAppers.com}" <i>(Creative Commons Attribution 3.0 License)</i>.
   */
  public MainFrame() {
    ((UncaughtExceptionHandler) Thread.getDefaultUncaughtExceptionHandler())
        .setParentComponent(panel.get());
    Dimension frameSize =
        new Dimension(
            Integer.parseInt(Configuration.INSTANCE.get("application.min.width")),
            Integer.parseInt(Configuration.INSTANCE.get("application.min.height")));

    Image iconImage =
        Toolkit.getDefaultToolkit()
            .getImage(getClass().getResource(Configuration.INSTANCE.get("application.icon.path")));
