...
/**
 * Responsible for window minimizing and closing. If SystemTray is supported, the window MainFrame
  ...
  public MainWindowListener(MainFrame mainFrame) {
    useTray =
        (SystemTray.isSupported()
            && Boolean.parseBoolean(Configuration.INSTANCE.get("application.tray.use")));

    if (useTray) {
      TrayPopup trayPopup = new TrayPopup(mainFrame);
