...
public final class MailSaver extends Observable {
   ...
   *
   * <p>The method will not copy the first 4 lines of the input stream.<br>
   ...
   *
   * <p>This lock will be used to make the application thread-safe, and avoid receiving and deleting
   * emails in the same time.
   *
   * @return a lock object <i>(which is actually the current instance of the {@code MailSaver}
   *     object)</i>.
   */
  ...
  private String saveEmailToFile(String mailContent) {
    ...
    String filePath =
        String.format(
            "%s%s%s",
            UIModel.INSTANCE.getSavePath(), File.separator, dateFormat.format(new Date()));
    ...
    try {
      ...
      Logger smtpLogger = LoggerFactory.getLogger(Session.class);
