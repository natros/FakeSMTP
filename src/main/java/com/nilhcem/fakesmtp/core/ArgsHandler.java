...
public enum ArgsHandler {
  ...
  private static final String OPT_BACKGROUNDSTART_DESC =
      "If specified, does not start the GUI. Must be used with the -"
          + OPT_AUTOSTART_SHORT
          + " (--"
          + OPT_AUTOSTART_LONG
          + ") argument";
  ...
  private static final String OPT_RELAYDOMAINS_DESC =
      "Comma separated email domain(s) for which relay is accepted. If not specified, relays to any"
          + " domain. If specified, relays only emails matching these domain(s), dropping (not"
          + " saving) others";
  ...
  private static final String OPT_MEMORYMODE_DESC =
      "Disable the persistence in order to avoid the overhead that it adds";
  private static final String OPT_BINDADDRESS_SHORT = "a";
  private static final String OPT_BINDADDRESS_LONG = "bind-address";
  private static final String OPT_BINDADDRESS_DESC =
      "IP address or hostname to bind to. Binds to all local IP addresses if not specified. Only"
          + " works together with the -"
          + OPT_BACKGROUNDSTART_SHORT
          + " (--"
          + OPT_BACKGROUNDSTART_LONG
          + ") argument.";
  ...
  ArgsHandler() {
    ...
    options.addOption(
        OPT_BACKGROUNDSTART_SHORT, OPT_BACKGROUNDSTART_LONG, false, OPT_BACKGROUNDSTART_DESC);
  ...
  public void handleArgs(String[] args) throws ParseException {
    ...
    if (memoryModeEnabled) {
      ((Logger) LoggerFactory.getLogger(Session.class)).setLevel(Level.INFO);
    }
  ...
  /**
   * @return whether or not the SMTP server must be running without a GUI, only if started at launch
   *     (if {@code shouldStartServerAtLaunch()} returns true}).
  ...
  /**
   * @return the output directory, as specified by the user, or a {@code null} string if
   *     unspecified.
   */
  ...
  /**
   * @return whether or not the SMTP server should disable the persistence in order to avoid the
   *     overhead that it adds. This is particularly useful when we launch performance tests that
   *     massively send emails.
   */
  ...
  /**
   * @return the name of executable used for viewing eml files, as specified by the user, or a
   *     {@code null} string if unspecified.
   */
